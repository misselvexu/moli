...
fn build_trusted_type_constructor<'s>(
    ...
    TrustedTypePrototypeDeclaration::new(v8str(scope, kind.as_slot_value()))
...
fn install_trusted_script_code_like_constructor<'s>(
    ...
    TrustedTypePrototypeDeclaration::new(v8str(scope, TrustedTypeKind::Script.as_slot_value()))
        .initialize(scope, prototype)
        .map_err(|error| {
...
fn trusted_type_to_string_callback<'s>(
    ...
    let this = args.this();
    if !get_private_value(scope, this, TRUSTED_TYPE_KIND_SLOT)
        .is_some_and(|kind| kind.strict_equals(args.data()))
    {
