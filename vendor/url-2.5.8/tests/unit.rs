...
#[test]
fn file_host_and_drive_letter_are_preserved_without_a_syntax_violation() {
    let violation = Cell::new(None);
    let url = Url::options()
        .syntax_violation_callback(Some(&|value| violation.set(Some(value))))
        .parse("file://mozilla.org/c:/file.txt")
        .unwrap();
    assert_eq!(url.as_str(), "file://mozilla.org/c:/file.txt");
